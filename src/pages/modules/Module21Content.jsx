...
import { MODULE21_QUESTIONS } from '../../data/module21Data';
import styles from './Module21Content.module.css';
...
const Module21Content = () => {
  ...
  const filteredQuestions = useMemo(() => {
    return MODULE21_QUESTIONS.map((q, originalIndex) => ({
      ...q,
      originalIndex
    })).filter(q => {
  ...
  const categoryCounts = useMemo(() => {
    const counts = { All: MODULE21_QUESTIONS.length };
    MODULE21_QUESTIONS.forEach(q => {
  ...
  return (
    <div className={styles.moduleLayout} onCopy={preventCopy}>
      <div className={styles.header}>
        <h1 className={styles.title}>Module 21 - Physical Design (PNR) Interview Questions</h1>
        <p className={styles.subtitle}>
          Master high-fidelity physical design and place-and-route interviews. Deep-dive into 151 questions covering inputs & setup, floorplanning, power planning, placement, clock tree synthesis (CTS), routing, signoff STA, and low power UPF closure.
        </p>
      </div>
      ...
      <div className={styles.statsBar}>
        <div className={styles.statBox}>
          ...
          <span className={styles.statValue}>{MODULE21_QUESTIONS.length}</span>
        </div>
        <div className={styles.statBox}>
...
export default Module21Content;
