...
const content = JSON.parse(fs.readFileSync('extracted_content_module5.json', 'utf8'));
...
const cleanHtml = (html) => {
    ...
    let cleaned = html
        ...
        .replace(/src="images\/image(\d+)\.png"/g, 'src="/assets/modules/module5/image_$1.png"');
...
mergedContent.forEach((item, index) => {
    ...
    if (item.type === 'img') {
        if (!item.src) return;
        const src = item.src.replace('images/image', '/assets/modules/module5/image_');
...
jsx = jsx.replace(/src=\\"images\/image(\d+)\.png/g, 'src=\\"/assets/modules/module5/image_$1.png');

fs.writeFileSync('c:\\Users\\priya\\vlsi-physical-design-ocean\\src\\pages\\modules\\module5Content.jsx', jsx);
console.log('Regenerated module5Content.jsx with absolute order preservation and MathJax $$ delimiters');
