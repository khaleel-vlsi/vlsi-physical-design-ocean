...
console.log('Reading PVandSignoff.html...');
const html = fs.readFileSync('scratch/module18/PVandSignoff.html', 'utf8');
  ...
  ch.elements.forEach(el => {
    ...
    $(el).find('img').each((j, img) => {
      ...
      if (src && src.startsWith('images/')) {
        ...
        $(img).attr('src', `/assets/modules/module18/${filename}`);
      }
    ...
    if ($(el).is('img')) {
      ...
      if (src && src.startsWith('images/')) {
        const filename = path.basename(src);
        $(el).attr('src', `/assets/modules/module18/${filename}`);
      }
      $(el).removeAttr('style');
...
const outputData = `export const MODULE18_CHAPTERS = ${JSON.stringify(finalChapters, null, 2)};\n`;
fs.writeFileSync('src/data/module18Data.js', outputData, 'utf8');
console.log('Done! Written to src/data/module18Data.js');
