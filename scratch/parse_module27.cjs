...
for (let i = 0; i < lines.length; i++) {
    ...
    if (line.match(/^#{20,}$/) && 
        ...
        i += 1;
        let j = i + 1;
        while(j < lines.length && (lines[j].match(/^#{20,}$/) || lines[j].startsWith('# Theory:') || lines[j].trim() === '')) {
             if (lines[j].trim() !== '') {
    ...
    if (currentChapter) {
        ...
        if (!currentChapter && line.match(/^#{20,}$/) === null) {
...
const outputStr = `export const MODULE27_CHAPTERS = ${JSON.stringify(finalData, null, 2)};\n`;
fs.writeFileSync('src/data/module27Data.js', outputStr);
console.log('Saved to src/data/module27Data.js');
